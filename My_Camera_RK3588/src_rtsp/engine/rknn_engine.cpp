...
// 全局计数器，用于为每个context分配NPU核心
static std::atomic<int> g_context_counter(0);

// 静态初始化：禁用 RKNN 调试日志 - 多种方法组合
static bool g_rknn_log_disabled = []() {
    // 方法1: 设置环境变量
    setenv("RKNN_LOG_LEVEL", "5", 1);  // 5 = ERROR level
    setenv("RKNN_PERF_DEBUG", "0", 1);  // 禁用性能调试
    
    // 方法2: 尝试动态加载 rknn 库并禁用日志
    void* handle = dlopen("librknnrt.so", RTLD_LAZY | RTLD_NOLOAD);
    if (handle) {
        // 查找日志设置函数（如果存在）
        typedef int (*rknn_set_log_level_func)(int);
        rknn_set_log_level_func set_log = (rknn_set_log_level_func)dlsym(handle, "rknn_set_log_level");
        if (set_log) {
            set_log(5); // 设置为 ERROR 级别
        }
    }
    
    NN_LOG_INFO("RKNN debug logs disabled");
    return true;
}();
...
nn_error_e RKEngine::LoadModelFile(const char *model_file)
{
    ...
    // ========================================
    // 关键修复：flag 必须设置为 0，不能使用 RKNN_FLAG_COLLECT_PERF_MASK
    // RKNN_FLAG_COLLECT_PERF_MASK (0x00000008) 会导致每次推理都输出详细的性能日志
    // ========================================
    ...
    // ========================================
    // 关键修改：为每个context绑定到不同的NPU核心
    // 使用轮询策略：Thread0→Core0, Thread1→Core1, Thread2→Core2, Thread3→Core0...
    // ========================================
    int ctx_id = g_context_counter.fetch_add(1);
    ...
    switch (ctx_id % 3) {
        case 0:
            ...
            NN_LOG_INFO("Context %d: Binding to NPU Core 0", ctx_id);
            break;
        case 1:
            core_mask = RKNN_NPU_CORE_1;
            NN_LOG_INFO("Context %d: Binding to NPU Core 1", ctx_id);
            break;
        case 2:
            core_mask = RKNN_NPU_CORE_2;
            NN_LOG_INFO("Context %d: Binding to NPU Core 2", ctx_id);
            break;
    }
    ...
    if (ret < 0)
    {
        NN_LOG_WARNING("rknn_set_core_mask failed, ret=%d, using default core", ret);
    }
    else
    {
        NN_LOG_INFO("NPU core mask set successfully for context %d", ctx_id);
    }

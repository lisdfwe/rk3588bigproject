...
// 读取视频帧，提交任务
void read_stream(const char *video_path1,const char *video_path2,const char *video_path3)
{
    // 读取视频
    cv::VideoCapture cap1(video_path1);
    cv::VideoCapture cap2(video_path2);
    cv::VideoCapture cap3(video_path3);

    if (!cap1.isOpened() ||!cap2.isOpened() ||!cap3.isOpened())
    {
        printf("Failed to open video file: %s, %s, %s\n", video_path1, video_path2, video_path3);
        //return -1;
    }
...
int main(int argc, char **argv)
{
    ...
    if (argc != 6)
    {
        printf("Usage: %s<model_path> <video_path1> <video_path2> <video_path3> <pool_num>\n", argv[0]);
        return -1;
    }
    ...
    char *stream_url3 = argv[4];   // 视频流地址 /dev/video4
    const int num_threads = (argc > 6) ? atoi(argv[5]) : 12;// 参数：线程池数量
    ...
    // 读取视频
    std::thread read_stream_thread(read_stream,  stream_url1, stream_url2 , stream_url3);
    // 启动结果线程
